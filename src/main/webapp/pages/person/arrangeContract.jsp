...
<head>
    ...
    <title><fmt:message key="head.contract"/></title>
            ...
            <div class="form-container">
                ...
                <table class="table">
                    <tr>
                        ...
                        <td>${temp_attribute.get("client").getLastname()} ${temp_attribute.get("client").getName()}</td>
                    </tr>
                    <tr>
                        <td><fmt:message key="trainer"/></td>
                        <td>${temp_attribute.get("trainer").getLastname()} ${temp_attribute.get("trainer").getName()}</td>
                    </tr>
                    <tr>
                        <td><fmt:message key="field.contract.start_date"/></td>
                        <td>${temp_attribute.get("start_date")}</td>
                    </tr>
                    <tr>
                        <td><fmt:message key="field.contract.end_date"/></td>
                        <td>${temp_attribute.get("end_date")}</td>
                    </tr>
                    <tr>
                        <td><fmt:message key="account_balance"/></td>
                        <td>${temp_attribute.get("client").getCash()}</td>
                    </tr>
                    <tr>
                        <td><fmt:message key="cost_contract"/></td>
                        <td>${temp_attribute.get("total_cost")}</td>
                    </tr>
                </table>
                <c:choose>
                    <c:when test="${temp_attribute.get('client').getCash()>=temp_attribute.get('total_cost')}">
                        <form action="${pageContext.request.contextPath}/controller" method="post">
                            <input type="hidden" name="command" value="sign_contract">
